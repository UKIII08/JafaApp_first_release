...
class _Event {
  final String title;
  final String type; // 'event', 'smallGroup', 'serviceMeeting', 'saturdayMeeting'
...
class _WydarzeniaCalendarScreenState extends State<WydarzeniaCalendarScreen> {
  ...
  void _addGeneralEvent(Map<String, dynamic> data, String collectionName, Map<DateTime, List<_Event>> eventsMap) {
    ...
    String type = "event"; // Domyślny typ

    if (collectionName == 'events') {
      // POPRAWKA 1: Naprawiony błąd - zmieniono 'date' na 'eventDate'
      eventDate = (data['eventDate'] as Timestamp?)?.toDate();
      title = data['title'] ?? 'Wydarzenie bez nazwy';

      // POPRAWKA 2: Dodana logika do rozpoznawania spotkań sobotnich
      final bool isSaturdayMeeting = data['sobota'] == true;

      if (isSaturdayMeeting) {
        type = 'saturdayMeeting'; // Specjalny typ dla spotkań sobotnich
      } else {
        type = 'event'; // Zwykłe wydarzenie
      }
              ...
              markerBuilder: (context, date, events) {
                ...
                return Row(
                  ...
                  children: eventTypes.take(4).map((type) {
                    ...
                    // POPRAWKA 3: Dodany wygląd dla spotkań sobotnich
                      case 'saturdayMeeting': color = Colors.orange; break;
              ...
              builder: (context, value, _) {
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    // POPRAWKA 4: Dodany wygląd dla listy spotkań sobotnich
                      case 'saturdayMeeting': color = Colors.orange; icon = Icons.calendar_today; break;
