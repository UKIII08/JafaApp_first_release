...
import 'package:intl/intl.dart'; // Upewnij się, że ten import jest
...
//import 'leaderboard_screen.dart'; // Dodaj import dla rankingu, jeśli go używasz w drawerze
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // --- Dodajemy instancje Firebase ---
  final FirebaseAuth _auth = FirebaseAuth.instance;
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  // --- Koniec dodawania instancji Firebase ---
  ...
  // --- VVV Funkcja sprawdzająca limit prób VVV ---
  Future<bool> _checkGameAttempts() async {
    final User? currentUser = _auth.currentUser;
    if (currentUser == null) {
      // Użytkownik niezalogowany - można pozwolić grać lub pokazać komunikat
      // Na razie pozwalamy grać, bo zapis wyniku i tak wymaga logowania
      return true;
    }
    ...
    try {
      ...
      if (snapshot.exists && snapshot.data() != null) {
        ...
        if (lastAttemptDate == todayDate) {
          // Jeśli data jest dzisiejsza, sprawdź liczbę prób
          if (dailyAttempts >= 3) {
            // Limit osiągnięty
            return false;
          }
        }
        // Jeśli data jest inna lub prób jest mniej niż 3, można grać
        return true;
      } else {
        // Jeśli dokument nie istnieje, użytkownik nie grał - można grać
        return true;
      }
    } catch (e) {
      print("Błąd sprawdzania limitu prób gry: $e");
      // W razie błędu lepiej pozwolić grać, niż blokować bez powodu
      return true;
    }
  }

  // --- VVV Funkcja pokazująca komunikat o limicie VVV ---
  ...
  Widget build(BuildContext context) {
    final user = _auth.currentUser; // Używamy instancji zdefiniowanej wyżej
      ...
      drawer: Drawer(
        child: ListView(
          ...
          children: [
            ...
            ListTile( leading: const Icon(Icons.local_library_outlined), title: const Text("Biblioteka"), onTap: () { Navigator.pop(context); Navigator.push( context, MaterialPageRoute(builder: (context) => const LibraryScreen()), ); }, ), // Istniejąca Biblioteka
            ...
            // --- VVV ZMODYFIKOWANY ListTile JAFA GAMES VVV ---
            ListTile(
              leading: const Icon(Icons.sports_esports_outlined), // Ikona dla gier
              title: const Text("Jafa Games"),
              onTap: () async { // Zmieniono na async
                Navigator.pop(context); // Zamknij szufladę od razu

                // Pokaż wskaźnik ładowania podczas sprawdzania limitu
                showDialog(
                  context: context,
                  barrierDismissible: false, // Nie zamykaj przez kliknięcie obok
                  builder: (BuildContext context) {
                    return const Center(child: CircularProgressIndicator());
                  },
                );

                bool canPlay = await _checkGameAttempts(); // Sprawdź limit

                if (mounted) { // Sprawdź mounted przed zamknięciem dialogu
                   Navigator.pop(context); // Zamknij dialog ładowania
                }


                if (canPlay && mounted) { // Sprawdź też mounted przed nawigacją
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => const JafaGamesScreen()),
                  );
                } else if (!canPlay && mounted) {
                  _showLimitReachedDialog(context); // Pokaż komunikat o limicie
                }
              },
            ),
            // --- ^^^ KONIEC ZMODYFIKOWANEGO ListTile ^^^ ---
            ...
            ListTile(
              ...
              onTap: () async {
                ...
                await _auth.signOut(); // Używamy instancji zdefiniowanej wyżej
            ...
            Expanded(
              child: RefreshIndicator(
                ...
                child: StreamBuilder<QuerySnapshot>(
                  stream: _firestore // Używamy instancji zdefiniowanej wyżej
