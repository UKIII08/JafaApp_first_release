// lib/screens/wydarzenia_screen.dart
// Nadal potrzebny, jeśli _fetchUserName jest w tym pliku
import 'package:flutter/material.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:intl/intl.dart';

// Import dla nowego ekranu szczegółów (utworzymy go w następnym kroku)
import 'wydarzenie_detail_screen.dart';

class WydarzeniaScreen extends StatefulWidget {
  const WydarzeniaScreen({super.key});

  @override
  State<WydarzeniaScreen> createState() => _WydarzeniaScreenState();
}

class _WydarzeniaScreenState extends State<WydarzeniaScreen> {
  ...
  // Funkcja _markAttendance (bez zmian - wersja z zapisem tylko Timestamp)
  ...
  // Funkcja _fetchUserName (nadal może tu być lub przenieść do DetailScreen)
  // Na razie zostawmy, może się przydać gdzie indziej, ale nie jest już używana bezpośrednio w tym build
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: _firestore
            ...
            .snapshots(),
        builder: (context, snapshot) { // Początek buildera StreamBuilder
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
          }
          if (snapshot.hasError) {
            print("Błąd ładowania wydarzeń: ${snapshot.error}");
          ...
          return ListView.builder( // Początek ListView.builder
            padding: const EdgeInsets.all(12.0),
            itemCount: eventDocs.length,
            itemBuilder: (context, index) { // Początek itemBuilder
              // --- Definicje zmiennych w itemBuilder (bez zmian) ---
              ...
              // --- ZMIANA: Cały Card opakowany w InkWell dla nawigacji ---
              return InkWell(
                onTap: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => WydarzenieDetailScreen(
                        eventId: eventId,
                        // Opcjonalnie przekaż podstawowe dane, by uniknąć migotania
                        // initialTitle: title,
                ...
                borderRadius: BorderRadius.circular(12.0), // Dopasuj do Card shape
                child: Card( // Początek Card
                  margin: const EdgeInsets.only(bottom: 16.0),
                  elevation: 3.0,
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12.0)),
                  clipBehavior: Clip.antiAlias,
                  child: Padding( // Początek Padding
                    padding: const EdgeInsets.all(16.0),
                    child: Column( // Początek Column
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [ // Początek listy children dla Column

                        // --- Szczegóły Wydarzenia (bez zmian) ---
                        ...
                        // Wyświetl tylko fragment opisu lub nic
                        if (description.isNotEmpty)
                          Text(
                            description,
                            style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: Colors.grey[800]),
                            maxLines: 2, // Pokaż tylko 2 linie opisu na liście
                        ...
                        // --- Sekcja Obecności Użytkownika (bez zmian) ---
                        ...
                        // --- Koniec Sekcji Obecności Użytkownika ---

                        // --- USUNIĘTA SEKCJA ADMINA ---

                      ], // <<<< Koniec listy children dla Column
                    ), // <<<< Koniec Column
                  ), // <<<< Koniec Padding
                ), // <<<< Koniec Card
              ); // <<<< Koniec InkWell (i return dla itemBuilder)
            }, // <<<< Koniec itemBuilder
          ); // <<<< Koniec ListView.builder
        }, // <<<< Koniec builder dla StreamBuilder
      ), // <<<< Koniec StreamBuilder
    ); // <<<< Koniec Scaffold
  } // <<<< Koniec metody build
} // <<<< Koniec klasy _WydarzeniaScreenState